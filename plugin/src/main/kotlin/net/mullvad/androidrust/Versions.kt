...
object Versions {
    ...
    init {
        val serial =
            Json.decodeFromStream<VersionsSerial>(
                this::class.java.getResourceAsStream("/versions.json")!!
            )
        PLUGIN_VERSION = VersionNumber.parse(serial.version)

        val matrix =
            serial.supportedVersions
                .map { entry -> android(entry.key) to entry.value.map { gradle(it) }.toSet() }
                .toMap()
    ...
    fun earliestMaybeSupportedAndroidVersion(): VersionNumber {
        ...
        return VersionNumber(earliestSupported.major, earliestSupported.minor, 0, "alpha")
    }
